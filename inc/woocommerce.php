 ...
 * @return void
 */
function _amp_reviews_woocommerce_setup() {
	add_theme_support( 'woocommerce' );
	add_theme_support( 'wc-product-gallery-zoom' );
	add_theme_support( 'wc-product-gallery-lightbox' );
	add_theme_support( 'wc-product-gallery-slider' );
}
add_action( 'after_setup_theme', 'amp_reviews_woocommerce_setup' );
 ...
 * @return void
 */
function _amp_reviews_woocommerce_scripts() {
	wp_enqueue_style( 'amp-reviews-woocommerce-style', get_template_directory_uri() . '/woocommerce.css' );
	...
	wp_add_inline_style( 'amp-reviews-woocommerce-style', $inline_font );
}
add_action( 'wp_enqueue_scripts', 'amp_reviews_woocommerce_scripts' );
...
add_filter( 'body_class', 'amp_reviews_woocommerce_active_body_class' );
...
add_filter( 'loop_shop_per_page', 'amp_reviews_woocommerce_products_per_page' );
...
add_filter( 'woocommerce_product_thumbnails_columns', 'amp_reviews_woocommerce_thumbnail_columns' );
...
add_filter( 'loop_shop_columns', 'amp_reviews_woocommerce_loop_columns' );
...
add_filter( 'woocommerce_output_related_products_args', 'amp_reviews_woocommerce_related_products_args' );

if ( ! function_exists( 'amp_reviews_woocommerce_product_columns_wrapper' ) ) {
	/**
...
add_action( 'woocommerce_before_shop_loop', 'amp_reviews_woocommerce_product_columns_wrapper', 40 );

if ( ! function_exists( 'amp_reviews_woocommerce_product_columns_wrapper_close' ) ) {
	/**
...
add_action( 'woocommerce_after_shop_loop', 'amp_reviews_woocommerce_product_columns_wrapper_close', 40 );
...
if ( ! function_exists( 'amp_reviews_woocommerce_wrapper_before' ) ) {
	/**
	 * Before Content.
	 *
...
add_action( 'woocommerce_before_main_content', 'amp_reviews_woocommerce_wrapper_before' );

if ( ! function_exists( 'amp_reviews_woocommerce_wrapper_after' ) ) {
	/**
	 * After Content.
	 *
...
add_action( 'woocommerce_after_main_content', 'amp_reviews_woocommerce_wrapper_after' );
 ...
 *
	<?php
		if ( function_exists( 'amp_reviews_woocommerce_header_cart' ) ) {
			_s_woocommerce_header_cart();
		}
	?>
 */

if ( ! function_exists( 'amp_reviews_woocommerce_cart_link_fragment' ) ) {
	/**
	 * Cart Fragments.
	 *
...
add_filter( 'woocommerce_add_to_cart_fragments', 'amp_reviews_woocommerce_cart_link_fragment' );

if ( ! function_exists( 'amp_reviews_woocommerce_cart_link' ) ) {
	/**
	 * Cart Link.
	 *
	...
	function _s_woocommerce_cart_link() {
		?>
		<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'amp_reviews' ); ?>">
			<?php
			$item_count_text = sprintf(
				/* translators: number of items in the mini cart. */
				_n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'amp_reviews' ),
...
>>>>>>> 0c8d53512675ab4143906795dd343051eed25261
